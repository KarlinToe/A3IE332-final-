...
$eid_bl = $_SESSION['user']['EmployeeID'];
$e_bl   = mysqli_real_escape_string($conn, $eid_bl);
$rw_bl  = mysqli_query($conn, "SELECT DISTINCT ToWarehouseID FROM LotCustodyEvent WHERE EmployeeID='$e_bl' AND ToWarehouseID IS NOT NULL");
$wids_bl = [];
while ($row_bl = mysqli_fetch_assoc($rw_bl)) { $wids_bl[] = "'" . mysqli_real_escape_string($conn, $row_bl['ToWarehouseID']) . "'"; }

// Batch list — filtered to batches ever stored at any of this staff member's warehouses
$where  = ['1=1'];
$params = [];
if (!empty($wids_bl)) {
    $in_bl = implode(',', $wids_bl);
    $where[] = "EXISTS (SELECT 1 FROM StoredIn si2 WHERE si2.VendorID=b.VendorID AND si2.BatchNumber=b.BatchNumber AND si2.WarehouseID IN ($in_bl))";
}
