...
$rw = mysqli_query($conn, "SELECT DISTINCT ToWarehouseID FROM LotCustodyEvent WHERE EmployeeID='$e' AND ToWarehouseID IS NOT NULL");
$wids = [];
while ($row = mysqli_fetch_assoc($rw)) { $wids[] = "'" . mysqli_real_escape_string($conn, $row['ToWarehouseID']) . "'"; }
$wids_in = !empty($wids) ? implode(',', $wids) : "''";
...
$where = ["(ce.ToWarehouseID IN ($wids_in) OR ce.FromWarehouseID IN ($wids_in))"];
if ($df)   { $df_s = mysqli_real_escape_string($conn,$df);   $where[] = "ce.EventTime >= '$df_s 00:00:00'"; }
if ($dt)   { $dt_s = mysqli_real_escape_string($conn,$dt);   $where[] = "ce.EventTime <= '$dt_s 23:59:59'"; }
if ($emp)  { $emp_s = mysqli_real_escape_string($conn,$emp); $where[] = "(e.FirstName LIKE '%$emp_s%' OR e.LastName LIKE '%$emp_s%' OR ce.EmployeeID='$emp_s')"; }
if ($cond && in_array($cond,['Seal Intact','Packaging Damaged'])) { $cond_s = mysqli_real_escape_string($conn,$cond); $where[] = "ce.ConditionConfirmed='$cond_s'"; }
...
$rows = [];
while ($row = mysqli_fetch_assoc($stmt)) { $rows[] = $row; }
