...
if ($sel_zone) {
    ...
    $today = app_today(); // e.g. '2026-03-16'
    $now   = app_now();   // e.g. '2026-03-16 00:00:00'

    // Lots in zone
    $stmt = $pdo->prepare("
        SELECT v.VendorName, si.BatchNumber, si.LotSeq, bl.LotVolume,
               b.ExpiryDate, DATEDIFF(b.ExpiryDate, '$today') AS days_until_expiry
        FROM StoredIn si
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE sr.WarehouseID=? AND sr.ZoneCode=?
          AND sr.ReadingTime >= DATE_SUB('$now', INTERVAL 7 DAY)
